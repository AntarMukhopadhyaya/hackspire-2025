...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={`fixed inset-x-0 top-0 z-40 w-full bg-transparent ${
        className || ""
      }`}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={`relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start px-4 py-2 lg:flex dark:bg-transparent ${
        visible ? "bg-black/60 rounded-full" : "bg-transparent"
      } ${className || ""}`}
    >
      {children}
    </motion.div>
